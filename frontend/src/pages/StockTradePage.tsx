...
import type { OrderRequest, OrderResponse, OrderType, TradeType } from '../types/order';
import { StocksApi, type StockItemOption, type StockPricePoint, type StockCandlePoint } from '../api/stocks';
import { PortfolioApi, type PortfolioSummary } from '../api/portfolio';
import CandlePriceChart from '../components/Chart/CandlePriceChart';
...
function StockTradePage() {
  ...
  const [range, setRange] = useState<'1M'|'3M'|'6M'|'1Y'|'5Y'|'ALL'>('5Y');
  ...
  const stockOptions = useMemo(() => {
    ...
    return base.filter(o => o.label.toLowerCase().includes(ql) || o.ticker.toLowerCase().includes(ql));
  }, [items, query]);

  useEffect(() => {
    if (!user) return;
    OrdersApi.getUserPending(user.userNo).then(setPending).catch(() => setPending([]));
  }, [user]);

  useEffect(() => {
    if (!user) return;
    PortfolioApi.getHoldings().then(setPortfolio).catch(() => setPortfolio(null));
  }, [user]);
  ...
  const canSubmit = !!user && selectedStockNo != null && volume !== '' && (orderType !== 'LIMIT' || price !== '');
  ...
  const edit = (o: OrderResponse) => {
    ...
    setSelectedTicker(stockOptions.find(s => s.stockNo === o.stockNo)?.ticker || '');
    setPrice(o.price ?? '');
    setVolume(o.volume ?? '');
  ...
  const [rangeIdx, setRangeIdx] = useState<{startIndex:number; endIndex:number} | null>(null);

  useEffect(() => {
    (async () => {
      if (!selectedStockNo) { setCandle([]); return; }
      const to = new Date();
      const from = new Date();
      switch (range) {
        ...
        case 'ALL': from.setFullYear(to.getFullYear() - 10); break; // 임시: 10년
      }
      const toStr = to.toISOString().slice(0, 10);
      const fromStr = from.toISOString().slice(0, 10);
      try {
        const points = await StocksApi.candleHistory(selectedStockNo, fromStr, toStr);
        setCandle(points);
  ...
  const dataWithMA = useMemo(() => {
    ...
    const ma5 = calcMA(5); const ma20 = calcMA(20); const ma60 = calcMA(60);
    return candle.map((d, i) => ({ ...d, ma5: ma5[i], ma20: ma20[i], ma60: ma60[i] }));
  }, [candle]);

  const viewData = useMemo(() => {
    const start = rangeIdx?.startIndex ?? 0;
    const end = rangeIdx?.endIndex ?? (dataWithMA.length ? dataWithMA.length - 1 : 0);
    return dataWithMA.slice(start, end + 1).map(d => ({...d, date: String(d.date)}));
  }, [dataWithMA, rangeIdx]);
      ...
      <div className="max-w-[1600px] mx-auto grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
        <div className="bg-slate-800 rounded-xl p-5 border border-slate-700 order-2 xl:col-span-2">
          ...
          <div className="flex gap-2 mb-4">
            {(['BUY','SELL'] as TradeType[]).map(s => (
              <button key={s} onClick={() => setSide(s)}
                className={`px-4 py-2 rounded ${side===s? 'bg-emerald-600':'bg-slate-700'}`}>
                {s==='BUY'?'매수':'매도'}</button>
            ))}
          </div>
          ...
          <div className="flex gap-2 mb-3">
            ...
            />
            <select value={selectedTicker} onChange={e=>onSelect(e.target.value)}
                    className="w-1/2 bg-slate-700 rounded px-3 py-2">
              <option value="" disabled>종목 선택</option>
              {stockOptions.map(opt => (
                <option key={opt.stockNo} value={opt.ticker}>{opt.label}</option>
              ))}
            </select>
          </div>
          ...
          {orderType==='LIMIT' && (
            <div className="mb-3">
              ...
              <input value={price} onChange={e=>setPrice(e.target.value===''?'':Number(e.target.value))}
                     type="number" className="w-full bg-slate-700 rounded px-3 py-2" />
            </div>
          )}
          ...
          {(orderType==='STOP_LOSS' || orderType==='TAKE_PROFIT') && (
            <div className="mb-3">
              ...
              <input value={triggerPrice} onChange={e=>setTriggerPrice(e.target.value===''?'':Number(e.target.value))}
                     type="number" className="w-full bg-slate-700 rounded px-3 py-2" />
            </div>
          )}
          ...
          {orderType==='TIME_LIMIT' && (
            <div className="mb-3">
              ...
              <input value={expiresAt} onChange={e=>setExpiresAt(e.target.value)}
                     type="datetime-local" className="w-full bg-slate-700 rounded px-3 py-2" />
            </div>
          )}

          <div className="mb-4">
            <label className="block text-sm mb-1">수량</label>
            <input value={volume} onChange={e=>setVolume(e.target.value===''?'':Number(e.target.value))}
                   type="number" className="w-full bg-slate-700 rounded px-3 py-2" />
          </div>

          <button disabled={!canSubmit || loading}
                  onClick={submit}
                  className={`w-full py-3 rounded font-bold ${side==='BUY'?'bg-rose-600':'bg-cyan-600'} disabled:opacity-50`}>
            {side==='BUY'? '현금매수':'현금매도'}
          </button>
            ...
            <table className="w-full text-sm">
              ...
              {pending.map(o => (
                <tr key={o.tradeNo} className="border-t border-slate-700">
                  <td className="p-2">{stockOptions.find(s=>s.stockNo===o.stockNo)?.label || `#${o.stockNo}`}</td>
                  <td className="p-2">{o.type}/{o.orderType}</td>
                  <td className="p-2 text-right">{o.price?.toLocaleString?.() ?? '-'}</td>
                  <td className="p-2 text-right">{o.volume}</td>
                  <td className="p-2">{o.status}</td>
                  <td className="p-2 text-right flex gap-2 justify-end">
                    <button className="px-3 py-1 rounded bg-slate-700" onClick={()=>edit(o)}>수정</button>
                    <button className="px-3 py-1 rounded bg-red-600" onClick={()=>cancel(o.tradeNo)}>취소</button>
                  </td>
                </tr>
              ))}
              {pending.length===0 && (
                <tr><td className="p-3 text-slate-400" colSpan={6}>미체결 주문이 없습니다.</td></tr>
              )}
              </tbody>
            </table>
          </div>
          ...
          <div className="flex gap-2 mb-3">
            {(['1M','3M','6M','1Y','5Y','ALL'] as const).map(r => (
              <button key={r} onClick={()=>setRange(r)}
                className={`px-3 py-1 rounded ${range===r? 'bg-amber-600 text-white':'bg-slate-700 text-slate-200'}`}>{r}</button>
            ))}
          </div>
          {viewData.length>0 ? (
            ...
            <p className="text-slate-400">종목을 선택하면 5개년 종가 차트를 표시합니다. 데이터가 없으면 수집 완료 후 다시 시도하세요.</p>
          )}
          <div className="mt-6">
            {viewData.length>0 && (
        ...
        {portfolio ? (
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
              ...
              <StatMini title="손익" value={portfolio.totalPnl} valueClass={portfolio.totalPnl>=0?'text-rose-400':'text-sky-400'} />
              <StatMini title="수익률" value={`${portfolio.totalPnlRate.toFixed(2)} %`} />
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                ...
                <tbody>
                  ...
                  {portfolio.holdings.length===0 && (
                    <tr><td className="p-3 text-slate-400" colSpan={6}>보유 종목이 없습니다.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
          ...
          <p className="text-slate-400">포트폴리오 불러오는 중 또는 데이터가 없습니다.</p>
        )}
      </div>
      ...
      <div className="max-w-[1600px] mx-auto mt-6">
        ...
        {portfolio ? (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <StatBig title="손익" value={portfolio.totalPnl} valueClass={portfolio.totalPnl>=0?'text-rose-400':'text-sky-400'} />
            <StatBig title="수익률" value={`${portfolio.totalPnlRate.toFixed(2)} %`} />
          </div>
        ) : (
          <p className="text-slate-400">포트폴리오 불러오는 중 또는 데이터가 없습니다.</p>
        )}
...
function StatMini({title, value, valueClass}:{title:string; value:any; valueClass?:string}){
  return (
    <div className="bg-slate-900 rounded-xl p-4 border border-slate-700">
      <div className="text-slate-400 text-sm">{title}</div>
      <div className={`text-lg font-bold ${valueClass??''}`}>{typeof value==='number'? value.toLocaleString(): value}</div>
    </div>
  );
}

function StatBig({title, value, valueClass}:{title:string; value:any; valueClass?:string}){
  return (
    <div className="bg-slate-800 rounded-xl p-5 border border-slate-700">
      <div className="text-slate-400 text-sm">{title}</div>
      <div className={`text-2xl font-extrabold ${valueClass??''}`}>{typeof value==='number'? value.toLocaleString(): value}</div>
    </div>
  );
}
