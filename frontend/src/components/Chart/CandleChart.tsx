import { ResponsiveContainer, ComposedChart, XAxis, YAxis, Tooltip, CartesianGrid, Bar, Customized, Line, Brush, Cell } from 'recharts';
...
export default function CandleChart({ data }: { data: Candle[] }) {
  ...
  const withMA = useMemo(() => {
    ...
    return data.map((d, i) => ({ ...d, ma5: ma5[i], ma20: ma20[i], ma60: ma60[i] }));
  }, [data]);
  ...
  const CandleLayer = (props: any) => {
    const { xAxisMap, yAxisMap, data: chartData, offset } = props ?? {};
  ...
  return (
    <div className="w-full h-[480px]">
      <ResponsiveContainer width="100%" height="100%">
        <ComposedChart data={view} margin={{ top: 10, right: 20, bottom: 10, left: 0 }} barCategoryGap={"70%"}>
          <CartesianGrid stroke="#334155" vertical={false} />
          <XAxis dataKey="date" stroke="#cbd5e1" tick={{ fontSize: 12 }} interval={Math.max(0, Math.floor(view.length / 10))} />
          <YAxis yAxisId="price" stroke="#cbd5e1" domain={[priceMin * 0.98, priceMax * 1.02]} />
          <YAxis yAxisId="vol" orientation="right" stroke="#94a3b8" allowDecimals={false} tickFormatter={(v) => (v as number).toLocaleString()} />
          ...
          {/* Range selector (zoom) */}
          <Brush dataKey="date" height={28} travellerWidth={10}
                 onChange={(x:any)=> setRange(x?.startIndex!=null && x?.endIndex!=null ? {startIndex:x.startIndex, endIndex:x.endIndex} : null)}
                 stroke="#94a3b8" fill="#0f172a" tickFormatter={(d:any)=>String(d)} />
        </ComposedChart>
      </ResponsiveContainer>
    </div>
  );
